...
module.exports = db => {
  router.get("/photos", (request, response) => {
    ...
    const whereClause = request.query.location ?  'WHERE photo.city=$1' : ''
    ...
    db.query(`
      ...
      ${whereClause};
    `, whereClause ? [request.query.location] : undefined ).then(({ rows }) => {
      response.json(rows[0].photo_data);
    });
  });


  // router.get("/photos/:location", (request, response) => {
  //   const protocol = request.protocol;
  //   const host = request.hostname;
  //   const port = process.env.PORT || 8001;
  //   const serverUrl = `${protocol}://${host}:${port}`;
  ...
  //   `, [request.params.location]).then(({ rows }) => {
  //     console.log();
  //     // console.log(rows[0].photo_data);
  //     response.json(rows[0].photo_data);
  ...
  // router.get("/photos/:location", (request, response) => {
  //  console.log(typeof request.params.location);
  //  response.end(request.params.location)
  // });
